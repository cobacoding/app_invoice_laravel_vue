...
class InvoiceController extends Controller
{
    ...
    public function update_invoice(Request $request, $id) {
        ...
        // Ambil dan decode item faktur dari request
        $invoiceItems = json_decode($request->invoice_item, true);
    
        // Hapus item faktur yang ada
        $invoice->invoice_items()->delete();
    
        // Iterasi melalui item yang diberikan dan buat ulang item tersebut
        foreach($invoiceItems as $item) {
            InvoiceItem::create([
                'product_id' => $item['product_id'],
                'invoice_id' => $invoice->id, // Gunakan ID faktur yang benar
